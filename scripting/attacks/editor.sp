...
public void ShowSpawns(Bombsite site) {
    g_EditingSite = site;
    Attacks_MessageToAll("Showing spawns for bombsite \x04%s.", SITESTRING(site));
    ...
    Attacks_MessageToAll("Found %d CT spawns.", ct_count);
    Attacks_MessageToAll("Found %d T spawns.", t_count);
}
...
public void AddSpawn(int client) {
    ...
    if (g_NumSpawns + 1 >= MAX_SPAWNS) {
        Attacks_MessageToAll("{DARK_RED}WARNING: {NORMAL}the maximum number of spawns has been reached. New spawns cannot be added.");
    ...
    Attacks_MessageToAll("Added %s spawn for %s (#%d).",
...
public void DeleteClosestSpawn(int client) {
    ...
    if (closest >= 0) {
        Attacks_MessageToAll("Deleted spawn #%d.", closest);
...
public void SaveSpawns() {
    WriteSpawns();
    Attacks_MessageToAll("Map spawns saved.");
}

public void ReloadSpawns() {
    g_NumSpawns = ParseSpawns();
    Attacks_MessageToAll("Imported %d map spawns.", g_NumSpawns);
}

public void DeleteMapSpawns() {
    g_NumSpawns = 0;
    Attacks_MessageToAll("All spawns for this map have been deleted");
}
...
public void MoveToSpawnInEditor(int client, int spawnIndex) {
    ...
    Attacks_Message(client, "Teleporting to spawn {GREEN}%d", spawnIndex);
    Attacks_Message(client, "   Team: {MOSS_GREEN}%s", TEAMSTRING(g_SpawnTeams[spawnIndex]));
    Attacks_Message(client, "   Site: {MOSS_GREEN}%s", SITESTRING(g_SpawnSites[spawnIndex]));
}
