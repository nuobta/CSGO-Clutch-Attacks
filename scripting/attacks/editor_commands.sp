public Action Command_EditSpawns(int client, int args) {
    if (g_hEditorEnabled.IntValue == 0) {
        Attacks_Message(client, "The editor is currently disabled.");
    ...
    if (!g_EditMode) {
        ...
        Attacks_MessageToAll("Edit mode launched, basic commands:");
        Attacks_MessageToAll("!edit to bring up the editor menu");
        Attacks_MessageToAll("!show <a/b> to display spawn points for a site");
        Attacks_MessageToAll("!new <ct/t> <a/b> to add a spawn point");
        Attacks_MessageToAll("!delete to delete the nearest spawn");
    }
...
public Action Command_SaveSpawns(int client, int args) {
    ...
    SaveSpawns();
    Attacks_Message(client, "Saved %d spawns.", g_NumSpawns);
    return Plugin_Handled;
}
...
public Action Command_AddSpawn(int client, int args) {
    ...
    if (!g_EditMode) {
        Attacks_Message(client, "You are not in edit mode.");
        return Plugin_Handled;
    }
