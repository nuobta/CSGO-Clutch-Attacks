static void GetConfigFileName(char[] buffer, int size) {
    ...
    BuildPath(Path_SM, buffer, size, "configs/attacks/%s.cfg", mapName);
}
...
public int ParseSpawns() {
    ...
    if (!FileExists(configFile)) {
        LogError("The attacks config file (%s) does not exist", configFile);
        return 0;
    }

    KeyValues kv = new KeyValues("Spawns");
    if (!kv.ImportFromFile(configFile) || !kv.GotoFirstSubKey()) {
        LogError("The attacks config file was empty");
        delete kv;
        return 0;
    }
    ...
    do {
        ...
        //kv.GetVector("playerammount", vec, NULL_VECTOR);
        //g_Spawnammount[spawn] = 1;
        kv.GetString("playerammount", sBuf, sizeof(sBuf));
        g_Spawnammount[spawn]=StringToInt(sBuf);

        kv.GetString("condition",  sBuf, sizeof(sBuf));
        g_Spawncondition[spawn] = StringToInt(sBuf);
 ...
 */
public int SelectSpawn(int team, bool bombSpawn, int playerammount, int condition) {
    ArrayList potentialSpawns = new ArrayList();
    for (int i = 0; i < g_NumSpawns; i++) {
        if (g_SpawnTeams[i] == team && !g_SpawnTaken[i] && g_Bombsite == g_SpawnSites[i] && g_Spawnammount[i]==playerammount && g_Spawncondition[i]==condition) {
    ...
    if (potentialSpawns.Length == 0) {
        ...
        if (bombSpawn) {
            ...
            return SelectSpawn(team, false, playerammount, condition);
        } else {
