...
public Plugin myinfo = {
    name = "CS:GO Attacks",
    author = "Nuobta",
    description = "CS:GO attack practice,base on splewis's attacks",
    version = PLUGIN_VERSION,
    url = "https://github.com/splewis/csgo-attacks"
};

public void OnPluginStart() {
    LoadTranslations("common.phrases");
    LoadTranslations("attacks.phrases");

    /** ConVars **/
    g_EnabledCvar = CreateConVar("sm_attacks_enabled", "1", "Whether the plugin is enabled");
    g_hAutoTeamsCvar = CreateConVar("sm_attacks_auto_set_teams", "1", "Whether attacks is allowed to automanage team balance");
    g_hEditorEnabled = CreateConVar("sm_attacks_editor_enabled", "1", "Whether the editor can be launched by admins");
    g_hMaxPlayers = CreateConVar("sm_attacks_maxplayers", "9", "Maximum number of players allowed in the game at once.", _, true, 2.0);
    g_hRatioConstant = CreateConVar("sm_attacks_ratio_constant", "0.425", "Ratio constant for team sizes.");
    g_hRoundsToScramble = CreateConVar("sm_attacks_scramble_rounds", "10", "Consecutive terrorist wins to cause a team scramble.");
    g_hRoundTime = CreateConVar("sm_attacks_round_time", "30", "Round time left in seconds.");
    g_hUseRandomTeams = CreateConVar("sm_attacks_random_teams", "0", "If set to 1, this will randomize the teams every round.");
    g_WarmupTimeCvar = CreateConVar("sm_attacks_warmuptime", "25", "Warmup time on map starts");

    g_EnabledCvar.AddChangeHook(EnabledChanged);

    /** Create/Execute attacks cvars **/
    AutoExecConfig(true, "attacks", "sourcemod/attacks");

    g_hCvarVersion = CreateConVar("sm_attacks_version", PLUGIN_VERSION, "Current attacks version", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_DONTRECORD);
    ...
    RegAdminCmd("sm_edit", Command_EditSpawns, ADMFLAG_CHANGEMAP, "Launches the attacks spawn editor mode");
    RegAdminCmd("sm_spawns", Command_EditSpawns, ADMFLAG_CHANGEMAP, "Launches the attacks spawn editor mode");

    RegAdminCmd("sm_new", Command_AddSpawn, ADMFLAG_CHANGEMAP, "Creates a new attacks spawn");
    RegAdminCmd("sm_newspawn", Command_AddSpawn, ADMFLAG_CHANGEMAP, "Creates a new attacks spawn");
    RegAdminCmd("sm_delete", Command_DeleteSpawn, ADMFLAG_CHANGEMAP, "Deletes the nearest attacks spawn");
    RegAdminCmd("sm_deletespawn", Command_DeleteSpawn, ADMFLAG_CHANGEMAP, "Deletes the nearest attacks spawn");
    RegAdminCmd("sm_deletemapspawns", Command_DeleteMapSpawns, ADMFLAG_CHANGEMAP, "Deletes all attacks spawns for the current map");

    RegAdminCmd("sm_show", Command_Show, ADMFLAG_CHANGEMAP, "Shows all attacks spawns in a bombsite");
    RegAdminCmd("sm_goto", Command_GotoSpawn, ADMFLAG_CHANGEMAP, "Goes to a attacks spawn");
    RegAdminCmd("sm_nearest", Command_GotoNearestSpawn, ADMFLAG_CHANGEMAP, "Goes to nearest attacks spawn");

    RegAdminCmd("sm_iteratespawns", Command_IterateSpawns, ADMFLAG_CHANGEMAP);
    RegAdminCmd("sm_reloadspawns", Command_ReloadSpawns, ADMFLAG_CHANGEMAP, "Reloads attacks spawns for the current map, discarding changes");
    RegAdminCmd("sm_savespawns", Command_SaveSpawns, ADMFLAG_CHANGEMAP, "Saves attacks spawns for the current map");
    ...
    g_hOnGunsCommand = CreateGlobalForward("Attacks_OnGunsCommand", ET_Ignore, Param_Cell);
    g_hOnPostRoundEnqueue = CreateGlobalForward("Attacks_OnPostRoundEnqueue", ET_Ignore, Param_Cell, Param_Cell);
    g_hOnPreRoundEnqueue = CreateGlobalForward("Attacks_OnPreRoundEnqueue", ET_Ignore, Param_Cell, Param_Cell);
    g_hOnTeamSizesSet = CreateGlobalForward("Attacks_OnTeamSizesSet", ET_Ignore, Param_CellByRef, Param_CellByRef);
    g_hOnTeamsSet = CreateGlobalForward("Attacks_OnTeamsSet", ET_Ignore, Param_Cell, Param_Cell, Param_Cell);
    g_OnFailToPlant = CreateGlobalForward("Attacks_OnFailToPlant", ET_Ignore, Param_Cell);
    g_OnRoundWon = CreateGlobalForward("Attacks_OnRoundWon", ET_Ignore, Param_Cell, Param_Cell, Param_Cell);
    g_OnSitePicked = CreateGlobalForward("Attacks_OnSitePicked", ET_Ignore, Param_CellByRef);
    g_OnWeaponsAllocated = CreateGlobalForward("Attacks_OnWeaponsAllocated", ET_Ignore, Param_Cell, Param_Cell, Param_Cell);
...
public void ExecConfigs() {
    ...
    g_SavedCvars = ExecuteAndSaveCvars("sourcemod/attacks/attacks_game.cfg");
}
...
public Action Command_ScrambleTeams(int client, int args) {
    if (g_Enabled) {
        ...
        Attacks_MessageToAll("%t", "AdminScrambleTeams", client);
    }
}
...
public Action PlacePlayer(int client) {
    ...
    if (Attacks_InWarmup() && nPlayers < g_hMaxPlayers.IntValue) {
        return Plugin_Continue;
    }
    ...
    ChangeClientTeam(client, CS_TEAM_SPECTATOR);
    Queue_Enqueue(g_hWaitingQueue, client);
    Attacks_Message(client, "%t", "JoinedQueueMessage");
    return Plugin_Handled;
}
...
public Action Event_PlayerSpawn(Event event, const char[] name, bool dontBroadcast) {
    ...
    randomcondition=GetRandomInt(1,nPlayers);
        ...
        randomcondition=GetRandomInt(1,5);
    }

    int client = GetClientOfUserId(event.GetInt("userid"));
    if (!IsValidClient(client) || !IsOnTeam(client) || g_EditMode || Attacks_InWarmup())
        return;
...
public Action Event_RoundPostStart(Event event, const char[] name, bool dontBroadcast) {
    ...
    if (!g_EditMode) {
        ...
        Attacks_MessageToAll("%t", "RetakeSiteMessage", SITESTRING(g_Bombsite), g_NumT, g_NumCT);
    }
...
public void UpdateTeams() {
    ...
    if (g_hAutoTeamsCvar.IntValue != 0) {
        // Ordinary team switching by attacks
    ...
    for (int i = 0; i < length; i++) {
        ...
        if (IsValidClient(client)) {
            Attacks_Message(client, "%t", "WaitingQueueMessage", g_hMaxPlayers.IntValue);
        }
    }
...
public void TerroristsWon() {
    ...
    if (g_WinStreak >= toScramble) {
        if (ScramblesEnabled()) {
            g_ScrambleSignal = true;
            Attacks_MessageToAll("%t", "ScrambleMessage", g_WinStreak);
        }
        g_WinStreak = 0;
    } else if (g_WinStreak >= toScramble - 3 && ScramblesEnabled()) {
        Attacks_MessageToAll("%t", "WinStreakAlmostToScramble", g_WinStreak, toScramble - g_WinStreak);
    } else if (g_WinStreak >= 3) {
        Attacks_MessageToAll("%t", "WinStreak", g_WinStreak);
    }
}

public void CounterTerroristsWon() {
    if (!g_bombPlanted && IsValidClient(g_BombOwner) && g_RoundCount >= 3) {
        Attacks_MessageToAll("%t", "FailedToPlant", g_BombOwner);
    ...
    if (g_WinStreak >= 3) {
        Attacks_MessageToAll("%t", "WinStreakOver", g_WinStreak);
    }
...
public Action PugSetup_OnSetupMenuOpen(int client, Menu menu, bool displayOnly) {
    ...
    if (g_Enabled) {
        menu.AddItem("disable_attacks", "Disable attacks", style);
    } else {
        menu.AddItem("enable_attacks", "Enable attacks", style);
    }
...
public void PugSetup_OnSetupMenuSelect(Menu menu, int client, const char[] selected_info, int selected_position) {
    if (StrEqual(selected_info, "disable_attacks")) {
        g_EnabledCvar.SetInt(0);
        PugSetup_GiveSetupMenu(client, false, selected_position);
    } else if (StrEqual(selected_info, "enable_attacks")) {
