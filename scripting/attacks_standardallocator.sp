...
public Plugin myinfo = {
    name = "CS:GO attacks: standard weapon allocator",
    author = "splewis",
...
public void OnPluginStart() {
    g_hCTRifleChoiceCookie = RegClientCookie("attacks_ctriflechoice", "", CookieAccess_Private);
    g_hTRifleChoiceCookie = RegClientCookie("attacks_triflechoice", "", CookieAccess_Private);
    g_hAwpChoiceCookie = RegClientCookie("attacks_awpchoice", "", CookieAccess_Private);
}
...
public void Attacks_OnGunsCommand(int client) {
    GiveWeaponsMenu(client);
}

public void Attacks_OnWeaponsAllocated(ArrayList tPlayers, ArrayList ctPlayers, Bombsite bombsite) {
...
static void SetNades(char nades[NADE_STRING_LENGTH], int rand) {
    switch(rand) {
        case 1: nades = "f";
        case 2: nades = "h";
        case 3: nades = "s";
...
public void WeaponAllocator(ArrayList tPlayers, ArrayList ctPlayers, Bombsite bombsite) {
    ...
    for (int i = 0; i < tCount; i++) {
        ...
        Attacks_SetPlayerInfo(client, primary, secondary, nades, health, kevlar, helmet, kit);
    }
